...
async function convertMusicXmlToPdfAndroid(xmlString, outputFileName = 'partitura.pdf') {
  ...
  try {
    ...
    // 1. Obtener la representación en Base64 directamente desde jsPDF
    const pdfBase64 = await html2pdf()
      .set(options)
      .from(container)
      .outputPdf('datauristring'); // Devuelve string data:application/pdf;base64,...

    // 2. Enviar el Base64 a Java a través del AndroidBridge
    if (window.AndroidBridge && window.AndroidBridge.onPdfFinalizado) {
      window.AndroidBridge.onPdfFinalizado(true, pdfBase64);
    }
  } catch (error) {
