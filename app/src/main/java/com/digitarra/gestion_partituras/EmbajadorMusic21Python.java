...
public class EmbajadorMusic21Python {
    ...
    public JSONObject getNotasDetalladas(Path rutaPartitura) throws JSONException {

        try (PyObject jsonNotas = py.getModule(RUTA_SCRIPT_LECTORNOTAS_PY).callAttr("json_notas_detalladas", rutaPartitura.toString())) {
            return new JSONObject(jsonNotas.toString());
        }
    }
    ...
    public void editaPartitura(Path rutaXML, JSONObject cambiosPartitura) throws Exception {

        PyObject resultado = py.getModule("EditorPartituras").callAttr("editar_partitura", rutaXML.toString(), cambiosPartitura.toString());

        boolean correcto = resultado.toBoolean();

        if (!correcto) {
            throw new IOException("EditorPartituras.py no pudo guardar la partitura.");
        }
    }

    public JSONObject getNotasEdicion(Path rutaPartitura) throws JSONException {
        try (PyObject jsonNotas = py.getModule(RUTA_SCRIPT_LECTORNOTAS_PY).callAttr("json_notas_edicion", rutaPartitura.toString())) {
